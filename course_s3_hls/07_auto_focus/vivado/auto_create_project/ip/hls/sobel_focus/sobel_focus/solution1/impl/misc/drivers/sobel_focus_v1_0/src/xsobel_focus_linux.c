...
typedef struct {
    ...
    XSobel_focus_uio_map maps[ MAX_UIO_MAPS ];
} XSobel_focus_uio_info;

/***************** Variable Definitions **************************************/
static XSobel_focus_uio_info uio_info;
...
static int uio_info_read_name(XSobel_focus_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSobel_focus_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSobel_focus_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSobel_focus_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSobel_focus_Initialize(XSobel_focus *InstancePtr, const char* InstanceName) {
	XSobel_focus_uio_info *InfoPtr = &uio_info;
